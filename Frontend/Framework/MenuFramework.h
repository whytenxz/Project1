...
namespace IdaLovesMe {
	...
	enum GuiFlags_ {
		...
		GuiFlags_InputPath		   = 1 << 19,
		GuiFlags_LuaTab			   = 1 << 20,
		GuiFlags_VisualsTab		   = 1 << 21
	};
	...
	struct GuiContext {
		...
		struct MenuTabAnimation {
			float indicator = 0.f;
			float fromIndicator = 0.f;
			float toIndicator = 0.f;
		...
		float DpiScale = 1.f;
		int ChildLayoutVersion = 0;
	};

	namespace ui {
		constexpr float MenuBaseWidth = 660.f;
		constexpr float MenuBaseHeight = 560.f;

		Vec2 GetMenuDefaultSize(float dpiScale = 0.f);
		void EnsureMainWindowMinimumSize();
		void ResetChildLayouts(int version);
		...
		void					 EndChild();
		void					 UpdateMainTabAnimation(int* selected, int tabTotal, float speedPercent, float deltaTime);
		int						 GetMainTabDisplayIndex();
		void					 DrawMainTabIndicator(int total);
		void					 ApplyDpiScale(float scale);
		float					 GetDpiScale();
		float					 Scale(float value);
		//-------Controls-------//
		void					 TabButton(const char* label, int* selected, int num, int total, GuiFlags flags = NULL);
		bool					 Checkbox(const char* label, bool* v, bool special = false, bool disabled = false);
		...
		void					 Image(LPDIRECT3DTEXTURE9 texture, const Vec2& size, GuiFlags flags = NULL);
		void					 InputText(const char* id, char* buffer, GuiFlags flags = NULL, size_t bufferSize = 64, const char* hint = NULL);
		void					 InputTextMultiline(const char* id, char* buffer, size_t bufferSize, const Vec2& size = Vec2(0, 220));
