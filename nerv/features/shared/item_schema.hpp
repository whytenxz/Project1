...
public:
	...
	std::vector<std::string> knife_name_storage;
	std::vector<std::string> glove_name_storage;
	std::vector<std::string> weapon_name_storage;
	std::vector<std::string> agent_ct_name_storage;
	std::vector<std::string> agent_t_name_storage;
	std::unordered_map<uint16_t, std::vector<std::string>> paint_kit_name_storage;
...
private:
	...
	void finalize_agent_list(std::vector<item_info_t>& agents, std::vector<std::string>& name_storage, std::vector<const char*>& names_cstr);
};

inline lazy_ptr<c_item_schema> g_item_schema;
