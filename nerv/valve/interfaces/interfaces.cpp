...
void c_interfaces::initialize()
{
	...
	m_engine_cvar = get_interface<i_engine_cvar>(&g_modules->m_modules.tier0_dll, xorstr_("VEngineCvar007"));
	if (!m_engine_cvar)
		LOG_ERROR(xorstr_("[-] Failed to get: VEngineCvar007"));

	m_engine_client = get_interface<i_engine_client>(&g_modules->m_modules.engine2_dll, xorstr_("Source2EngineToClient001"));
	if (!m_engine_client)
		LOG_ERROR(xorstr_("[-] Failed to get: Source2EngineToClient001"));
}
